...
const Utility = require('./Utility Legacy.js');
...
module.exports.emoji_use = async (member, message) => {
    if (emoji_wait_list.indexOf(member.id) === -1) {
        if (message.content.search(/<:\w+:\d+>/g) != -1) {
            ...
            emoji_list = emoji_list.filter((item, i, arr) => {
                //Отсеивание одинаковых эмоджи
            ...
            for (let i = 0; i < emoji_list.length; i++) {
                if (!usersDB.find({ userID: member.id }).get('emoji').value()) {
                    ...
                    if (
                        usersDB.find({ userID: member.id }).get('emoji').value()[0][
                            emoji_list[i]
                        ] == undefined
                    ) {
                        //Если такое эмоджи не использовалось, то добавляем его
...
for (
    let i = 1, level = 2, progression = 50;
    i < 1000;
    i++, level++, progression = progression + 6
) {
...
let levelSys = async (member, message) => {
    ...
    if (usersDB.find({ userID: member.id }).value()) {
        if (usersDB.find({ userID: member.id }).value().exp != undefined) {
            ...
            if (
                usersDB.find({ userID: member.id }).value().lvl != undefined &&
                usersDB.find({ userID: member.id }).value().prestige != undefined
            ) {
                ...
                for (let i = 0; true; i++) {
                    if (exp >= lvl_list[i] && exp < lvl_list[i + 1]) {
                        usersDB
                            ...
                            .assign({ lvl: i + 1, prestige: (i + 1 - ((i + 1) % 100)) / 100 })
                            .write();
                        break;
                    }
                }
...
module.exports.statistics = async (guild) => {
    ...
    if (!guildDB.value()) {
        ...
        guildDB
            .assign({ members: guild.memberCount, messages: guildDB.value().messages + 1 })
            .write();
...
module.exports.addcolor = async (message, color) => {
    ...
    role.edit({
        ...
        .then((role) => {
            console.log(
                `Edit role with name ${role.name} and color ${role.color} by ${message.member.nickname}`
            );
...
module.exports.clearOldEmoji = async (message) => {
    ...
    members.forEach((member) => {
        if (usersDB.find({ userID: member.id }).get('emoji').value() != undefined) {
            let emoji_keys = Object.keys(
                usersDB.find({ userID: member.id }).get('emoji').value()[0]
            );
            emoji_keys.forEach((item) => {
                if (
                    message.guild.emojis.get(item.match(/\w:\d+/g)[0].match(/\d+/g)[0]) == undefined
                ) {
...
module.exports.stats = async (member, message) => {
    if (usersDB.find({ userID: member.id }).value()) {
        await levelSys(member, message);
    } else message.channel.send('Такого пользователя нет на сервере');
    ...
    if (usersDB.find({ userID: member.id }).get('emoji').value()) {
        ...
        if (sortable.length > 0) {
            sortable = sortable.filter((item) => {
                //Проверка на то, есть ли эмоджи на сервере(в целом не нужна)
                if (member.guild.emojis.get(item[0].match(/\w:\d+/g)[0].match(/\d+/g)[0]))
                    return true;
                else return false;
            });
...
module.exports.serverinfo = async (member, message) => {
    ...
    member.guild.members.forEach((user) => {
        //Подсчет пользователей онлайн и тд
    ...
    usersDB.value().forEach((item) => {
        //Парсинг списка юзеров и включение использованых ими эмоджи в массив объектов
        if (item.emoji != undefined && item.emoji.length > 0) {
            emoji_arr.push(item.emoji[0]);
        }
    });
    emoji_arr.reduce((a, o) => {
        //превращение массива в один объект без дублирующихся значений
        Object.keys(o).map((k) => (a[k] = (a[k] || 0) + o[k]));
        return a;
    }, o1);

    for (let emoji in o1) {
        //создание массива массивов с двумя значениями, 1-е название эмоджи 2-е количество использований
...
async function create_top_list(array, message, member, name, namefield) {
    ...
    array.forEach(async (item, i) => {
        //Перебираем массив и выводим топ
