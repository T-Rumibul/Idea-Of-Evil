...
export function Emit(oldPresence: Presence | null, newPresence: Presence, client: IOEClient) {
	const { member } = newPresence;
	if (!member) return;
	if (member.user.bot) return;

	if (newPresence.activities[0] && newPresence.activities[0].type === ActivityType.Playing) {
