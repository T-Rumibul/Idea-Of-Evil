...
export class Player extends BaseModule {
	...
	async reactionHandler(
		...
		try {
			const msg = this.playerControllMessages.get(
			...
			switch (reaction.emoji.name) {
				...
				case this.reactions[6]:
					...
					if (Oldconnection.joinConfig.channelId == newChannel.id)
						break;
	...
	sendChooseMessage(
		...
		return new Promise(async (resolve, reject) => {
			try {
				let embed: typeof embedTemplate = JSON.parse(
					JSON.stringify(embedTemplate)
				);
				...
				for (let i = 0; i < tracks.length; i++) {
					...
					embed.description += `${i + 1}. **[${tracks[i].title}](${
						tracks[i].url
					})** \n`;
				}
		...
		try {
			...
			if (message.content.match(urlRegEx)) {
				...
				if (validateUrl == 'sp_track') {
					...
					videoDetails = (await ytdl.video_basic_info(url))
						.video_details;
				...
				let selectedTrack = await this.sendChooseMessage(
					message.member,
	...
	async updateControllMessage(guildId: string) {
		...
		for (let i = 0; i < queue.length; i++) {
			if (i < 11) {
				if (i === 0) {
					if (queue[i].repeat)
						newEmbed.title = `🔁 [${queue[i].duration}] ${queue[i].title}`;
					else
						newEmbed.title = `[${queue[0].duration}] ${queue[i].title}`;
	...
	async removeFromQueue(guildId: string) {
		try {
			let queue: Queue = {};
			if (db.exists('/queue'))
				queue = await db.getObject<Queue>('/queue');
			if (queue.hasOwnProperty(guildId)) {
	...
	async play(message: Message) {
		...
		if (message.channel.type !== ChannelType.GuildText) return;
		try {
			if (message.channelId !== this.channels.get(message.guildId))
				return;
			if (
				(await this.client.checkBlackListUser(message.author.id)) !==
				null
			) {
				const msg = await message.channel.send(
					`<@${
	...
	async connect(guildId: string, connection?: VoiceConnection) {
		...
		connection.on('stateChange', (old_state, new_state) => {
			this.log(
				'join',
				`Connection state change from', ${old_state.status}, 'to', ${new_state.status}`
			);
			if (
