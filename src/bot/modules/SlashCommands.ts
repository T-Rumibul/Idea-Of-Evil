...
/**
 * The constructor takes in an IOEClient as a parameter and calls the init()
...
export class SlashCommands extends Base {
  ...
  /**
   * Registers commands for each guild in the client's guilds cache, then
   * registers events for when an interaction is created or when a new guild is
   * created.
   */
  ...
  /**
   * Method takes in an interaction and finds the command associated with it,
   * then executes it with the client as a parameter.
   */
...
/**
 * SlashCommands() is exported as a default function which creates an instance
 * of SlashCommands if one does not already exist and returns it.
 */
