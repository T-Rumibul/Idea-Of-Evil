...
export class Welcomer extends Base {
  ...
  async sendWelcomeMesssageTrigger(member: GuildMember, client: IOEClient) {
    ...
    const channelId = await client.IOE.externalDB.guild.getWelcomeChannel(
      member.guild.id
    );
  ...
  async sendWelcomeMesssage(member: GuildMember, channel: TextChannel) {
    ...
    const AVATAR_MASK = await jimp.read(
      path.join(ASSETS_PATH, 'avatar-mask.png')
    );
    const NICKNAME_FONT = await jimp.loadFont(
      path.join(ASSETS_PATH, 'nickname.fnt')
    );
    ...
    username =
      username.charAt(0).toUpperCase() + username.slice(1, 11).toLowerCase();
